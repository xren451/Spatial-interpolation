...
temperature_data = station_value[:, :500, 0]
...
def train_and_predict(station_coords, temperature_data, epochs=100, batch_size=10, learning_rate=0.1, patience=10):
    ...
    for target_index in range(num_stations):
        ...
        mlp_all_model = MLP_model(station_coords.shape[0] * station_coords.shape[1], num_stations * d_model)  # Adjust input_dim to match station_coords
        mlp_one_model = MLP_model(station_coords.shape[1], d_model)  # Adjust input_dim to match station_coords
        moe_model = MoE(num_experts=num_subgraphs, output_dim=d_model)

        all_coords = station_coords.flatten().astype(np.float32).reshape(1, -1)  # Ensure float32 type
        H_pos_all = mlp_all_model.predict(all_coords).reshape((num_stations, d_model))
        H_pos_one = mlp_one_model.predict(target_coords.astype(np.float32).reshape(1, -1)).reshape((d_model,))
        ...
        for epoch in range(epochs):
            ...
            for batch in range(num_batches):
                    ...
                    for i, gcn_model in enumerate(gcn_models):
                        ...
                        for b in range(batch_size):
                            for n in range(num_stations):
                                ukfs[i].predict()
                                ukfs[i].update(H_gcn_reshaped[b, n, :].numpy())
                                ukf_features.append(ukfs[i].x)
                        ukf_features = np.array(ukf_features).reshape(batch_size, num_stations, -1)
                ...
                # Gather trainable variables from all GCN models and the MoE model
                trainable_vars = []
                for gcn_model in gcn_models:
                    trainable_vars.extend(gcn_model.trainable_variables)
                trainable_vars.extend(moe_model.trainable_variables)
