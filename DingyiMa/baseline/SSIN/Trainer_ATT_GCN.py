...
from SSIN.dataset_collator.create_data_att_gcn import *
...
#         self.r=r
#         inter_channels = int(self.num_features // self.r * self.num_timestamps)

#         inter_channels_global=num_features // r
        

#         self.relu1=nn.ReLU(inplace=True)
#         self.GCN1 = GCNConv(self.num_timestamps * self.num_features, inter_channels)
#         self.GCN2 = GCNConv(inter_channels, self.num_timestamps * self.num_features)
...
#             nn.Conv2d(num_features, inter_channels_global, kernel_size=1, stride=1, padding=0),
#             nn.BatchNorm2d(inter_channels_global),
#             nn.ReLU(inplace=True),
#             nn.Conv2d(inter_channels_global, num_features, kernel_size=1, stride=1, padding=0),
...
#         # print("Input x shape:", x_gcn.shape)  # 检查输入的形状
...
class MaskedTrainer:
    def __init__(self, args, global_step=0, out_path=None, init_training=True):
        ...
        # load data
        self.all_seq_data, self.all_seq_fusion,self.invalid_masks_data, self.r_pos_mat_data, self.adj_attn_mask, self.edge_index = self.load_train_data()
    ...
    def load_train_data(self):
        ...
        edge_index=data_dict["edg_index"]
        ...
        return all_seq_data,all_seq_fusion, invalid_masks_data, r_pos_mat_data, adj_attn_mask, edge_index
    ...
    def load_test_data_generator(self):
        ...
        test_data=test_data_expanded[:,5:,:,:]
        test_tensor = torch.from_numpy(test_data)  # Batch size 8784, 18 features, 41 sites, width 1
        test_tensor = test_tensor.float().cuda()
        edge_index_tensor = edge_index.cuda()
        output_test_tensor = self.fusion_model(test_tensor,edge_index_tensor)
    ...
    def train(self):
        ...
        try:
            ...
            for epoch in range(1, self.args.epochs+1):
                ...
                # print(f"train_tensor device: {train_tensor.device}")
                # print(f"edge_index_tensor device: {edge_index_tensor.device}")
                # print(f"fusion_model device: {next(self.fusion_model.parameters()).device}")
                output_tensor = self.fusion_model(train_tensor,edge_index_tensor)
