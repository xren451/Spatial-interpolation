...
# ------------------------------------- For training data ------------------------------------- #
def create_train_data(epoch, output_numpy,all_seq_data, invalid_masks, batch_size, masked_lm_prob, times=1, adj_attn_mask=None):
    """times: create how many times of the whole data"""
    print("use att model_creat_train")
        ...
        for t in range(times):
            ...
            for i in range(batch_num):
                ...
                for idx in batch_indexes:  # todo: modify batch, concatenate
                    ...
                    seq_data_fusion = output_numpy[idx]
                    ...
                    masked_seq, masked_indexes, masked_labels, masked_label_weights, attn_mask = \
                        create_random_masked_data(seq_data_fusion,seq_data, seq_len, invalid_mask, max_pred_per_seq, masked_lm_prob)
...
def create_random_masked_data(seq_data_fusion,seq_data, seq_len, invalid_mask, max_pred_per_seq, masked_lm_prob=0.15):
    masked_seq = seq_data_fusion.copy()
    ...
    mean_value = np.mean(seq_data_fusion[unmasked_indexes, :1])
    std_value = np.std(seq_data_fusion[unmasked_indexes, :1])
    mean_value_label = np.mean(seq_data[unmasked_indexes, :1])
    std_value_label = np.std(seq_data[unmasked_indexes, :1])
    ...
    if std_value_label == 0:  # Discard the seqs: after normalization, will generate nan or inf value
        masked_labels = np.array(masked_labels).astype(float) - mean_value_label  # standardize label
    else:
        masked_labels = (np.array(masked_labels).astype(float) - mean_value_label) / std_value_label  # standardize label
...
def create_test_data(all_seq_data, output_test_numpy,invalid_masks, test_masks, all_timestamps, adj_attn_mask=None):
    data_num = len(all_seq_data)
    seq_len = all_seq_data.shape[1]
    all_indexes = range(data_num)
    print("use att model_creat_test")

    with tqdm(total=data_num, desc=f'Testing:') as pbar:
        for idx in all_indexes:
            seq_data = all_seq_data[idx]
            seq_data_fusion = output_test_numpy[idx]
            ...
            masked_seq, masked_indexes, masked_labels, attn_mask, mean_value, std_value = \
                create_masked_data_by_idx(seq_data, seq_data_fusion,seq_len, invalid_mask, test_mask)
...
def create_masked_data_by_idx(seq_data, fusion_data,seq_len, invalid_mask, test_mask):
    """For test data"""
    masked_seq = fusion_data.copy()
    full_indexes = set(range(seq_len))
    ...
    mean_value = np.mean(fusion_data[unmasked_indexes, :1])
    std_value = np.std(fusion_data[unmasked_indexes, :1])
    ## 在这里
    # mean_value_label = np.mean(seq_data[unmasked_indexes, :1])
    # std_value_label = np.std(seq_data[unmasked_indexes, :1])
    ...
    ## 在这里
    # if std_value_label == 0:  # Discard the seqs: after normalization, will generate nan or inf value
    #     masked_labels = masked_labels - mean_value_label  # normalization
    # else:
    #     masked_labels = (masked_labels - mean_value_label) / std_value_label  # normalization
    # # masked_seq[masked_indexes, :1] = 0  # set test nodes as 0
